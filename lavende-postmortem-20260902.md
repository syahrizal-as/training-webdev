...
## TheLavendeResidence (103.134.154.19) — 2 September 2026

**Severity:** SEV-1 (Data loss produksi, partial permanent)
**Durasi dampak:** ±2,5 jam (17:05–19:30 WIB, app maintenance mode)
...
Pada 2 September 2026 sekitar pukul **17:05 WIB**, proses verifikasi patch webhook hardening (Laravel) di server produksi TheLavende menjalankan perintah:
...
**Hasil akhir:**
- ✅ `products` (tabel utama, 329.182 baris) — **PULIH 100%** dari backup parsial yang sempat dibuat 12 menit sebelum insiden
...
- ✅ Aplikasi kembali normal ±19:30 WIB setelah recovery + hardening oleh OpenClaw

---

## 2. Kronologi Lengkap (WIB)

| Waktu | Kejadian |
|---|---|
| 13:17 | Hermes pasang observability Nginx + PHP-FPM (webhook timing) di Lavende |
| 16:43 | OpenClaw update patch v2 (10 file) di `/tmp/webhook-hardening.patch` |
| 16:56 | Hermes buat backup **parsial**: `mysqldump lavende products` (94,8 MB) + `.env` → `/root/hermes-backups/lavende-deploy-qiscus-20260902-165639/` |
| 17:04 | OpenClaw apply patch v2 ke working tree `/var/www/lavende` + isi `.env` QISCUS_* |
| 17:05–17:10 | Hermes commit `1e0bd7f` + `config:clear` |
| 17:11 | Hermes jalankan `php artisan test tests/Feature/QiscusWebhookTest.php` **di production** |
| 17:11–17:15 | `RefreshDatabase` → `migrate:fresh` → **DROP 19 tabel produksi** → migrate ulang terputus (timeout 180 detik) |
| 17:15 | Hermes sadari tabel hilang → cek state → konfirmasi insiden |
| 17:26 | Hermes stop supervisor worker + maintenance mode |
| 17:27 | Hermes backup binlog 34 file (301 MB, 4 Agu–2 Sep) → `/root/hermes-backups/lavende-binlog-20260902-172749/` |
| 17:34–17:38 | MySQL restart (proses tidak terkontrol), sempat connection refused |
| 17:40 | Recovery: restore baseline dump 2022 (`project_apart.sql`) → 16 tabel struktur + data lama |
| 17:45 | Migrate upgrade: 8 migration 2023–2025 DONE; 2 migration gagal (butuh kolom `webhook`) |
| 17:45 | Import `products` live terpotong (LOCK TABLES privilege) |
| 18:30–19:30 | OpenClaw (Insight) takeover recovery: sanitasi dump, restore products 329.182 baris, perbaiki struktur, pasang guard |
| 19:15 | Backup harian otomatis pertama berjalan (32 MB .sql.gz) |
| ±19:30 | App UP, worker RUNNING, recovery selesai |
...
```text
1. Backup parsial products 16:56 (94,8 MB) — dibuat 12 menit sebelum
   insiden karena task deploy migration index products
   → SATU-SATUNYA alasan 329.182 baris data utama selamat

2. Backup binlog 34 file (301 MB, 4 Agu–2 Sep) — dibuat 17:27
...
5. Import products live dari backup 16:56 (329.182 baris)
...
| Backup harian full DB | Cron `0 2 * * *` → `/usr/local/bin/backup_lavende.sh` (mysqldump full + gzip, retensi 2 file) | ✅ Aktif — backup pertama 32 MB (19:15) |
...
✅ Backup harian aktif (cron 02:00 + backup pertama sudah ada)
...
*Report disusun oleh Hermes Agent sebagai bentuk pertanggungjawaban penuh atas insiden. Data didasarkan pada log sesi, artefak backup di `/root/hermes-backups/`, dan verifikasi langsung ke server.*
